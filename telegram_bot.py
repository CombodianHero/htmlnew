...
def parse_txt_file(file_path):
    ...
    for line in lines:
        ...
        # Skip HTML content (if file is already HTML)
        ...
        # Handle formats like: "Lect-1 Basic of Networks" or "Lect.-1 EVS"
        subject_match = re.search(r'(?:Lect[.-]?\d+\s+)(.+?)(?:\s*\(|$)', title)
        if subject_match:
            subject_name = subject_match.group(1).strip()
        else:
            # Try to get everything after lecture number
            subject_match = re.search(r'Lect[.-]?\d+\s+(.+)', title)
            if subject_match:
                subject_name = subject_match.group(1).strip()
            else:
                # If no lecture format, use the whole title as subject
                subject_name = title if len(title) < 50 else title[:50] + "..."
        ...
        if is_video:
            subjects_dict[subject_name]['videos'].append({
                'title': title,
                'src': url,
                'drm': 'classplus' in match.group(2 if category else 1).lower()  # Check original URL
            })
        else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📤 Send me a .txt file with ONE of these formats:\n\n"
        "Format 1: (Category)Title:URL\n"
        "Format 2: Title: URL\n\n"
    ...
    try:
        ...
        if not parsed_data or len(parsed_data) == 0:
            await update.message.reply_text(
                ...
                "Make sure your file has lines in ONE of these formats:\n"
                "• (Category)Title:URL\n"
                "• Title: URL\n\n"
                "Example:\n"
                "Lect-1 Basic of Networks: https://example.com/video.m3u8"
            )
        ...
        total_videos = sum(len(s['subjects'][0]['videos']) for s in parsed_data)
        total_pdfs = sum(len(s['subjects'][0]['pdfs']) for s in parsed_data)
...
def main():
    ...
    logger.info("✨ Now supports BOTH file formats:")
    logger.info("   • (Category)Title:URL")
